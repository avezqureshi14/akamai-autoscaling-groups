<template>
    <el-card class="container mx-auto" body-class="flex flex-col space-y-10">
        <div class="flex justify-between items-center">
            ...
            <div class="flex flex-row space-x-2">
                <el-button type="primary" icon="Plus" @click="showAddAutoScalingGroupDialog = true">Add Auto Scaling Group</el-button>
        ...
        <el-table :data="autoScalingGroups" v-loading="loading" class="w-full" stripe>
            ...
            <el-table-column prop="state" label="State" align="center" width="100">
                <template #default="scope">
                    ...
                    <el-tag type="warning" v-else-if="scope.row.state == 'REFRESHING'">{{ scope.row.state }}</el-tag>
                    <el-tag type="danger" v-else-if="scope.row.state == 'IN_ROLLBACK'">{{ scope.row.state }}</el-tag>
            ...
            <el-table-column label="Launch Template" align="center" width="320">
                <template #default="scope">
                    <el-descriptions size="small" :column="1" border>
                        <el-descriptions-item align="right" :label="launchTemplates.filter(launchTemplates => launchTemplates.id === scope.row.lt_id).length > 0 ? launchTemplates.find(launchTemplates => launchTemplates.id === scope.row.lt_id).name : scope.row.lt_id"><b>v</b>{{ scope.row.lt_version }}</el-descriptions-item>
                        <el-descriptions-item align="right" label="Network Threshold">{{ scope.row.network_threshold }}</el-descriptions-item>
                    </el-descriptions>
                </template>
            </el-table-column>
            <el-table-column label="Network" align="center" width="250">
                <template #default="scope">
                    <el-descriptions size="small" :column="1" border>
                        <el-descriptions-item align="right" label="VPC">{{ vpcs.filter(vpc => vpc.id === scope.row.vpc_id).length > 0 ? vpcs.find(vpc => vpc.id === scope.row.vpc_id).label : scope.row.vpc_id }}</el-descriptions-item>
                        <el-descriptions-item align="right" label="Subnet">{{ vpcs.filter(vpc => vpc.id === scope.row.vpc_id).length > 0 ? vpcs.find(vpc => vpc.id === scope.row.vpc_id).subnets.find(subnet => subnet.id === scope.row.subnet_id).label : scope.row.subnet_id }}</el-descriptions-item>
                    </el-descriptions>
                </template>
            </el-table-column>
            <el-table-column label="Time" align="center" width="250">
                <template #default="scope">
                    <el-descriptions size="small" :column="1" border>
                        <el-descriptions-item align="right" label="Last Refresh">{{ timeAgo(scope.row.last_refresh_completed_at.Time) }}</el-descriptions-item>
                        <el-descriptions-item align="right" label="Created">{{ timeAgo(scope.row.created_at.Time) }}</el-descriptions-item>
                    </el-descriptions>
                </template>
            </el-table-column>
            <el-table-column label="Actions" width="200" align="center" fixed="right">
                <template #default="scope">
                    ...
                    <el-button type="danger" icon="Delete" size="small" @click="confirmDeleteAutoScalingGroup(scope.row)"></el-button>
                </template>
            </el-table-column>
        </el-table>
    ...
    <!-- Add Dialog -->
    <el-dialog title="Add Auto Scaling Group" v-model="showAddAutoScalingGroupDialog" class="bg-white rounded-md" append-to-body>
        <el-form :model="newAutoScalingGroup" label-position="top">
            ...
            <el-form-item label="Launch Template ID">
                <el-select v-model="newAutoScalingGroup.launch_template_id" placeholder="Select Launch Template" @change="filterLaunchTemplateVersions('new')">
            ...
            <el-form-item label="Launch Template Version">
                <el-select v-model="newAutoScalingGroup.launch_template_version" placeholder="Select Launch Template Version">
                    <el-option v-for="lt in filteredNewLaunchTemplates" :key="lt.version" :label="lt.version" :value="lt.version"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="VPC ID">
                <el-select v-model="newAutoScalingGroup.vpc_id" placeholder="Select VPC" v-if="filteredVpcs.length">
                    <el-option v-for="vpc in filteredVpcs" :key="vpc.id" :label="vpc.label" :value="vpc.id"></el-option>
                </el-select>
            ...
            <el-form-item label="Subnet ID" v-if="newAutoScalingGroup.vpc_id">
                <el-select v-model="newAutoScalingGroup.subnet_id" placeholder="Select Subnet">
                    <el-option v-for="subnet in filteredSubnets" :key="subnet.id" :label="subnet.label + ' ('+subnet.ipv4+')'" :value="subnet.id"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item>
            ...
            <div class="grid grid-cols-2">
                ...
                <el-form-item label="Max Instance Capacity">
                    <el-input-number v-model="newAutoScalingGroup.max_desired_capacity" :min="newAutoScalingGroup.min_desired_capacity+1" />
                </el-form-item>
        ...
        <template #footer>
            ...
            <el-button type="primary" @click="createAutoScalingGroup">Create</el-button>
        </template>
    </el-dialog>

    <!-- Edit Dialog -->
    <el-dialog title="Edit Auto Scaling Group" v-model="showEditAutoScalingGroupDialog" class="bg-white rounded-md" append-to-body>
        <el-form :model="editingAutoScalingGroup" label-position="top">
            <el-form-item label="Name">
                <el-input v-model="editingAutoScalingGroup.name" disabled/>
            </el-form-item>
            <el-form-item label="Launch Template ID">
                <el-select v-model="editingAutoScalingGroup.launch_template_id" placeholder="Select Launch Template" @change="filterLaunchTemplateVersions('edit')" disabled>
                    <el-option v-for="lt in launchTemplates" :key="lt.id" :label="lt.name" :value="lt.id"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="Launch Template Version">
                <el-select v-model="editingAutoScalingGroup.launch_template_version" placeholder="Select Launch Template Version">
                    <el-option v-for="lt in filteredEditLaunchTemplates" :key="lt.version" :label="lt.version" :value="lt.version"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="VPC ID">
                <el-select v-model="editingAutoScalingGroup.vpc_id" placeholder="Select VPC" v-if="filteredVpcs.length">
                    <el-option v-for="vpc in filteredVpcs" :key="vpc.id" :label="vpc.label" :value="vpc.id"></el-option>
                </el-select>
                <el-input v-else v-model="editingAutoScalingGroup.vpc_id" placeholder="Enter VPC ID"></el-input>
            </el-form-item>
            ...
            <div class="grid grid-cols-2">
                ...
                <el-form-item label="Max Instance Capacity">
                    <el-input-number v-model="editingAutoScalingGroup.max_desired_capacity" :min="editingAutoScalingGroup.min_desired_capacity+1" :max="100"/>
                </el-form-item>
                <el-form-item label="CPU Threshold (%)">
        ...
        <template #footer>
            ...
            <el-button type="primary" @click="updateAutoScalingGroup">Update</el-button>
        </template>
    </el-dialog>

    <!-- Delete Confirmation Dialog -->
    <el-dialog title="Delete Auto Scaling Group" v-model="showDeleteAutoScalingGroupDialog" class="bg-white rounded-md" append-to-body>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            filterLaunchTemplateVersions(type) {
                if (type === 'new') {
                    ...
                    if (this.filteredNewLaunchTemplates.length === 1) {
                        this.newAutoScalingGroup.launch_template_version = this.filteredNewLaunchTemplates[0].version;
                    }
                    ...
                    if (this.filteredEditLaunchTemplates.length === 1) {
                        this.editingAutoScalingGroup.launch_template_version = this.filteredEditLaunchTemplates[0].version;
                    }
                    this.filterVpcsByRegion();
                }
            },
            ...
            fetchVpcs() {
                this.loading = true;
                axios.get('https://api.linode.com/v4/vpcs', {
                    headers: {
            ...
            editAutoScalingGroup(autoScalingGroup) {
                ...
                .then((response) => {
                    ...
                    if (response.data.ssl_certificate_cred_id) {
                        this.editingAutoScalingGroup.ssl_certificate_cred_id = response.data.ssl_certificate_cred_id;
                    }
            ...
            manage(asg) {
                this.$router.push({ name: 'AutoScalingGroupManagement', params: { asgId: asg.id.replace("asg:", "") } });
            },
        },
    };
</script>